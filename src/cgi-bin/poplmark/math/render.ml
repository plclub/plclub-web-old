...
let cmd_convert tmpprefix finalpath = "convert -transparent white -quality 100 -density 120 " ^ tmpprefix ^ ".ps " ^ finalpath ^ " >/dev/null 2>/dev/null"
...
let render tmppath finalpath outtex md5 =
	...
	else if (Sys.command ("chmod a+r " ^ (finalpath^"/"^md5^".png")) != 0)
	then (unlink_all (); raise (ExternalCommandFailure "chmod"))
	else unlink_all ()
      end
